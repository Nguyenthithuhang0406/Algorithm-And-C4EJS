...
int main(){
	...
	for(int i = 1; i <= n; i++){
		cin >> a[i];
		L[i] = 1; // ban đầu coi mỗi phần tử là 1 dãy nên L[i] = 1;
	}
	for(int i = 1; i <= n; i++){
		...
		for(int j = 1; j < i; j++){
			if(a[j] < a[i]){
				L[i] = max(L[i], L[j] + 1);   
				// nếu số thứ j nhỏ hơn số thứ i thì có thể thêm số thứ i vào dãy con kết thúc là số thứ j
				//độ dài dãy con sẽ chọn số lớn nhất trong 2 trường hợp
                //(nếu thêm số i vào dãy j : L[i] = L[j] + 1; neu khong them vao: L[i] = L[i])
			}
		}
	}
...
/*bài toán cái túi,cái túi có thể chưá được tối đa trọng lượng là S, có n món đồ,
 mỗi món đồ có trọng lượng w và giá trị v, bạn có thể lựa chọn tuỳ ý sao cho tổng giá trị là lớn nhất*/
#include<bits/stdc++.h>
using namespace std;
